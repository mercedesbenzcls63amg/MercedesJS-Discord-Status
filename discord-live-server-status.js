...
export default class DiscordLiveServerStatus extends BasePlugin {
  ...
  resolveEmojiForTeam(teamName, factionCode) {
    ...
    if (teamName) {
      ...
      for (const [key, emoji] of Object.entries(this.resolvedEmojiMap)) {
        if (emoji && lower.includes(key)) return emoji;
      }

      for (const [synonym, mapKey] of Object.entries(FACTION_SYNONYMS)) {
        if (lower.includes(synonym) && this.resolvedEmojiMap[mapKey]) {
  ...
  async buildMessage() {
    const s = this.options.strings;
    const colorInt = parseInt(String(this.options.embedColor).replace('#', ''), 16) || 0xbf0000;

    // ── Embed ──
    const embed = new EmbedBuilder();
    embed.setTitle(s.title);
    embed.setDescription(
      s.matchStarted.replace('{time}', this.getTimeAgo(this.server.matchStartTime))
    );
    embed.setColor(colorInt);
    embed.setTimestamp();
    embed.setFooter({ text: COPYRIGHT_MESSAGE });
    ...
    embed.addFields(
      ...
      {
        name: s.queue,
        value: `${s.publicQueue}: **${publicQueue}**\n${s.whitelist}: **${reserveQueue}**`,
        inline: true
      },
      { name: '\u200b', value: '\u200b', inline: false },
      {
        name: `${td.team1.emoji ? td.team1.emoji + ' ' : ''}${td.team1.name}`,
        value: `${s.player}: **${td.team1.players}**\n${s.squad}: **${td.team1.squads}**`,
        inline: true
      },
      {
        name: `${td.team2.emoji ? td.team2.emoji + ' ' : ''}${td.team2.name}`,
